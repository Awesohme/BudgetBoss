-- BudgetBoss Database Setup
-- Run this in your Supabase SQL Editor to set up the database

-- First, ensure auth schema exists and has proper permissions
-- (This is usually already set up in Supabase)
...
-- Fixed expenses (keeping for compatibility)
...
CREATE TABLE IF NOT EXISTS categories (
    ...
    notes TEXT, -- breakdown notes and reminders
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    deleted BOOLEAN DEFAULT FALSE
);
...
DROP POLICY IF EXISTS "Users can read shared budgets" ON budgets;
DROP POLICY IF EXISTS "Users can insert own budgets" ON budgets;
DROP POLICY IF EXISTS "Budget owners can update" ON budgets;
DROP POLICY IF EXISTS "Budget owners can delete" ON budgets;
...
DROP POLICY IF EXISTS "Users can read budget memberships" ON budget_members;
DROP POLICY IF EXISTS "Budget owners can manage members" ON budget_members;
...
DROP POLICY IF EXISTS "Budget members can read incomes" ON incomes;
DROP POLICY IF EXISTS "Budget members can manage incomes" ON incomes;
...
-- Drop existing policies for fixed_expenses
DROP POLICY IF EXISTS "Budget members can read fixed expenses" ON fixed_expenses;
DROP POLICY IF EXISTS "Budget members can manage fixed expenses" ON fixed_expenses;
...
DROP POLICY IF EXISTS "Budget members can read categories" ON categories;
DROP POLICY IF EXISTS "Budget members can manage categories" ON categories;
...
DROP POLICY IF EXISTS "Budget members can read transactions" ON transactions;
DROP POLICY IF EXISTS "Budget members can manage transactions" ON transactions;
...
AS $$
BEGIN
    INSERT INTO public.settings (user_id, currency)
    VALUES (NEW.id, 'NGN')
